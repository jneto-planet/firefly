...
export function registerFireflyIpc() {
  ...
  ipcMain.handle("firefly:pick-file", async (_e, options?: { 
    ...
    if (options?.fileType === 'executable') {
      ...
      filters = [
        { name: 'Firmware Package', extensions: ['zip'] },
        { name: 'All Files', extensions: ['*'] }
      ];
    } else {
      filters = [{ name: 'All Files', extensions: ['*'] }];
  ...
  ipcMain.handle("firefly:open-proxy-tool", async () => {
    try {
      const config = await loadConfig();
      const scriptPath = config.proxy_tool_path?.trim();

      if (!scriptPath) {
        console.error("[firefly] Proxy Tool path not configured");
        dialog.showErrorBox(
          "Proxy Tool Not Configured",
          "No Proxy Tool path has been set. Please configure it in Settings."
        );
        return false;
      }

      console.log(`[firefly] Launching Proxy Tool from: ${scriptPath}`);
      ...
      console.log("[firefly] Proxy Tool launched successfully");
      return true;
    } catch (error) {
      console.error("[firefly] Failed to open Proxy Tool:", error);
      dialog.showErrorBox(
        "Failed to Launch Proxy Tool",
        `An error occurred while launching Proxy Tool: ${error instanceof Error ? error.message : String(error)}`
      );
      return false;
    }
  });
  ...
  // --- Open OPI Simulator ---
  ipcMain.handle("firefly:open-opi-simulator", async () => {
    try {
      const config = await loadConfig();
      const scriptPath = config.opi_simulator_path?.trim();

      if (!scriptPath) {
        console.error("[firefly] OPI Simulator path not configured");
        dialog.showErrorBox(
          "OPI Simulator Not Configured",
          "No OPI Simulator path has been set. Please configure it in Settings."
        );
        return false;
      }

      console.log(`[firefly] Launching OPI Simulator from: ${scriptPath}`);
      ...
      console.log("[firefly] OPI Simulator launched successfully");
      return true;
    } catch (error) {
      console.error("[firefly] Failed to open OPI Simulator:", error);
      dialog.showErrorBox(
        "Failed to Launch OPI Simulator",
        `An error occurred while launching OPI Simulator: ${error instanceof Error ? error.message : String(error)}`
      );
      return false;
    }
  });
  ...
  ipcMain.handle("firefly:install-app", async (_e, { serial, apkPath, allowDowngrade, packageName }: { serial: string; apkPath: string; allowDowngrade?: boolean; packageName?: string }) => {
    try {
      // For a version downgrade, Android requires a clean install: uninstall first.
      if (allowDowngrade && packageName) {
        await adbs(serial, "uninstall", packageName);
      }
  ...
  // ─── APK version comparison (shared by Apps + Firmware) ────────────────────
  ipcMain.handle("firefly:check-apk-version", async (_e, { serial, apkPath }: { serial: string; apkPath: string }) => {
    try {
      const apkInfo = extractApkVersionInfo(apkPath);
      if (!apkInfo.package) {
        return { success: false, error: "Could not read package info from APK" };
      }
      const installed = await getInstalledVersion(serial, apkInfo.package);
      const isDowngrade =
        installed.installed &&
        apkInfo.versionCode != null &&
        installed.versionCode != null &&
        apkInfo.versionCode < installed.versionCode;
      return {
        success: true,
  ...
  // ─── Firmware ──────────────────────────────────────────────────────────────
  ipcMain.handle("firefly:firmware-validate", async (_e, { zipPath }: { zipPath: string }) => {
    let tempDir: string | null = null;
    try {
      const baseName = path.basename(zipPath);
      if (!baseName.toLowerCase().endsWith(".zip")) {
        return { success: false, error: "Selected file must be a .zip archive" };
      }
      const expectedLabel = baseName.slice(0, -4); // strip ".zip"

      tempDir = path.join(os.tmpdir(), `firefly_fw_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`);
      await fs.mkdir(tempDir, { recursive: true });

      const zip = new AdmZip(zipPath);
      zip.extractAllTo(tempDir, true);

      // manifest.xml must exist at the archive root
      const manifestPath = path.join(tempDir, "manifest.xml");
      if (!(await pathExists(manifestPath))) {
        await safeRemoveFirmwareTemp(tempDir);
        return { success: false, error: "manifest.xml not found — invalid firmware package" };
      }

      const manifestXml = await fs.readFile(manifestPath, "utf8");
      const parsed = parseFirmwareManifest(manifestXml);

      if (!parsed.label) {
        await safeRemoveFirmwareTemp(tempDir);
        return { success: false, error: "manifest.xml is missing a label attribute" };
      }
      if (parsed.label !== expectedLabel) {
        await safeRemoveFirmwareTemp(tempDir);
        return {
          success: false,
          error: `Label mismatch — manifest label "${parsed.label}" does not match file name "${expectedLabel}"`,
        };
      }
      if (parsed.bundles.length === 0) {
        await safeRemoveFirmwareTemp(tempDir);
        return { success: false, error: "manifest.xml contains no bundles" };
      }

      // Every bundle file must be present in the package
      const bundles: Array<{ filename: string; order: number; md5: string | null; reboot: boolean; apkPath: string }> = [];
      for (const b of parsed.bundles) {
        const apkPath = path.join(tempDir, b.filename);
        if (!(await pathExists(apkPath))) {
          await safeRemoveFirmwareTemp(tempDir);
          return { success: false, error: `Bundle file missing in package: ${b.filename}` };
        }
        bundles.push({ ...b, apkPath });
      }
      bundles.sort((a, b) => a.order - b.order);

      return { success: true, label: parsed.label, tempDir, bundles };
    } catch (e) {
      if (tempDir) await safeRemoveFirmwareTemp(tempDir);
      return { success: false, error: e instanceof Error ? e.message : String(e) };
    }
  });

  ipcMain.handle("firefly:firmware-verify-md5", async (_e, { apkPath, expectedMd5 }: { apkPath: string; expectedMd5?: string }) => {
    try {
      const buf = await fs.readFile(apkPath);
      const actual = crypto.createHash("md5").update(buf).digest("hex").toLowerCase();
      const match = expectedMd5 ? actual === expectedMd5.toLowerCase() : true;
      return { success: true, match, actual };
    } catch (e) {
      return { success: false, match: false, actual: "", error: e instanceof Error ? e.message : String(e) };
    }
  });

  ipcMain.handle("firefly:firmware-reboot-wait", async (_e, { serial, timeoutMs = 60_000 }: { serial: string; timeoutMs?: number }) => {
    try {
      const start = Date.now();
      await adbs(serial, "reboot");
      // Give the device a moment to actually drop offline before we start waiting.
      await delay(3000);
      // Block until adb sees the device again (bounded by runAsync's internal timeout).
      await adbs(serial, "wait-for-device");
      // wait-for-device returns before Android has fully booted; poll boot_completed.
      while (Date.now() - start < timeoutMs) {
        const r = await adbs(serial, "shell", "getprop", "sys.boot_completed");
        if (r.code === 0 && r.out.trim() === "1") {
          return { success: true };
        }
        await delay(2000);
      }
      return { success: false, error: `Device did not come back online within ${Math.round(timeoutMs / 1000)}s` };
    } catch (e) {
      return { success: false, error: e instanceof Error ? e.message : String(e) };
    }
  });

  ipcMain.handle("firefly:firmware-cleanup", async (_e, { tempDir }: { tempDir: string }) => {
    await safeRemoveFirmwareTemp(tempDir);
    return { success: true };
  });
}
...
/** Only deletes directories that look like our firmware temp dirs, to be safe. */
async function safeRemoveFirmwareTemp(dir: string): Promise<void> {
  try {
    if (dir && path.basename(dir).startsWith("firefly_fw_")) {
...
/** Parse the (plain-text) firmware manifest.xml. */
function parseFirmwareManifest(xml: string): { label: string | null; bundles: FirmwareBundle[] } {
  const labelMatch = xml.match(/<manifest[^>]*\blabel\s*=\s*"([^"]*)"/i);
  const label = labelMatch ? labelMatch[1] : null;

  const bundles: FirmwareBundle[] = [];
  const bundleRegex = /<bundle\b[^>]*?\/?>/gi;
  let m: RegExpExecArray | null;
  while ((m = bundleRegex.exec(xml)) !== null) {
    const tag = m[0];
    const filename = (tag.match(/\bfilename\s*=\s*"([^"]*)"/i) || [])[1];
    if (!filename) continue;
    const orderStr = (tag.match(/\border\s*=\s*"([^"]*)"/i) || [])[1];
    const md5 = (tag.match(/\bmd5\s*=\s*"([^"]*)"/i) || [])[1] || null;
    const rebootStr = (tag.match(/\breboot\s*=\s*"([^"]*)"/i) || [])[1];
    bundles.push({
      filename,
      order: orderStr ? parseInt(orderStr, 10) : 0,
      md5,
      reboot: (rebootStr || "").toLowerCase() === "true",
    });
  }
  return { label, bundles };
}

/** Read package / versionCode / versionName from an APK's binary AndroidManifest.xml. */
function extractApkVersionInfo(apkPath: string): { package: string | null; versionCode: number | null; versionName: string | null } {
  const empty = { package: null, versionCode: null, versionName: null };
  try {
    const zip = new AdmZip(apkPath);
    const entry = zip.getEntry("AndroidManifest.xml");
    if (!entry) return empty;
    const buf = entry.getData();
    if (buf.length < 8 || buf.readUInt16LE(0) !== 0x0003) return empty;

    let pos = 8;
    let strings: string[] = [];
    const result: { package: string | null; versionCode: number | null; versionName: string | null } = {
      package: null,
      versionCode: null,
      versionName: null,
    };
      ...
      if (chunkType === 0x0001) {
        ...
        if (elemName === "manifest") {
          ...
          for (let i = 0; i < attrCount; i++) {
            ...
            if (attrName === "package") {
              if (dataType === 0x03) result.package = strings[data] ?? null;
            } else if (attrName === "versionCode") {
              result.versionCode = data;
            } else if (attrName === "versionName") {
              if (dataType === 0x03) result.versionName = strings[data] ?? null;
            }
          }
          break; // manifest is the root element
        }
      }
      pos += chunkSize;
    }
...
/** Read the installed version of a package from the device via dumpsys. */
async function getInstalledVersion(
  serial: string,
  pkg: string
): Promise<{ versionCode: number | null; versionName: string | null; installed: boolean }> {
  try {
    const r = await adbs(serial, "shell", "dumpsys", "package", pkg);
    if (r.code !== 0 || !r.out) return { versionCode: null, versionName: null, installed: false };
    const vcMatch = r.out.match(/versionCode=(\d+)/);
    const vnMatch = r.out.match(/versionName=([^\s\n]+)/);
    const installed = vcMatch != null || vnMatch != null;
    if (!installed) return { versionCode: null, versionName: null, installed: false };
    return {
    ...
    return { versionCode: null, versionName: null, installed: false };
  }
}
