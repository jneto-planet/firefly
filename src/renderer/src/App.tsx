...
export default function App() {
  ...
  const [proxyToolPath, setProxyToolPath] = React.useState<string>("");
  const [opiSimulatorPath, setOpiSimulatorPath] = React.useState<string>("");
  ...
  // Proxy Tool state
  const [openingProxyTool, setOpeningProxyTool] = React.useState(false);

  // OPI Simulator state
  const [openingOpiSimulator, setOpeningOpiSimulator] = React.useState(false);
  ...
  async function boot() {
    try {
      ...
      setProxyToolPath(cfg?.proxy_tool_path || "");
      setOpiSimulatorPath(cfg?.opi_simulator_path || "");
  ...
  async function handleOpenProxyTool() {
    if (!proxyToolPath) {
      setShowSettings(true);
      return;
    }
    try {
      setOpeningProxyTool(true);
      const success = await window.firefly.openProxyTool();
      if (!success) {
        console.error("Failed to open Proxy Tool");
      }
      // Keep feedback for 2 seconds
      setTimeout(() => setOpeningProxyTool(false), 2000);
    } catch (e) {
      console.error("Error opening Proxy Tool:", e);
      setOpeningProxyTool(false);
    }
  }

  async function handleOpenOpiSimulator() {
    if (!opiSimulatorPath) {
      setShowSettings(true);
      return;
    }
    try {
      setOpeningOpiSimulator(true);
      const success = await window.firefly.openOpiSimulator();
      if (!success) {
        console.error("Failed to open OPI Simulator");
      }
      // Keep feedback for 2 seconds
      setTimeout(() => setOpeningOpiSimulator(false), 2000);
    } catch (e) {
      console.error("Error opening OPI Simulator:", e);
      setOpeningOpiSimulator(false);
    }
  }
  ...
  function SettingsDialog() {
    ...
    async function save() {
      try {
        ...
        // Save proxy tool path
        configUpdates.proxy_tool_path = proxyToolPath;

        // Save OPI Simulator path
        configUpdates.opi_simulator_path = opiSimulatorPath;
    ...
    async function browseProxyTool() {
      try {
        const path = await window.firefly.pickFile({
          title: "Select Proxy Tool Script",
          fileType: "any",
        });
        if (path) {
          setProxyToolPath(path);
        }
      } catch (e) {
        alert(`Failed to open file picker: ${e}`);
      }
    ...
    async function browseOpiSimulator() {
      try {
        const path = await window.firefly.pickFile({
          title: "Select OPI Simulator Script",
          fileType: "any",
        });
        if (path) {
          setOpiSimulatorPath(path);
        }
      } catch (e) {
        alert(`Failed to open file picker: ${e}`);
      }
        ...
        >
          ...
          <div className="space-y-4">
            <div className="border-t pt-4" style={{ borderColor: "rgba(255,255,255,0.1)" }}>
              ...
              <div className="flex items-center justify-between mb-4">
                <div>
                  <div className="text-sm font-medium text-white">Proxy Tool Path</div>
                  <div className="text-xs text-white/60">Path to the Proxy Tool script</div>
                  {proxyToolPath && (
                    <div className="text-xs text-white/40 truncate" style={{ maxWidth: '200px' }} title={proxyToolPath}>Path: {proxyToolPath}</div>
                  )}
                </div>
                <button
                  onClick={browseProxyTool}
                  className="px-3 py-1 text-xs rounded border"
                  style={{ borderColor: "rgba(255,255,255,0.12)", color: "#fff" }}
                >
                  Select...
              ...
              <div className="flex items-center justify-between mb-4">
                <div>
                  <div className="text-sm font-medium text-white">OPI Simulator Path</div>
                  <div className="text-xs text-white/60">Path to the OPI Simulator script</div>
                  {opiSimulatorPath && (
                    <div className="text-xs text-white/40 truncate" style={{ maxWidth: '200px' }} title={opiSimulatorPath}>Path: {opiSimulatorPath}</div>
                  )}
                </div>
                <button
                  onClick={browseOpiSimulator}
                  className="px-3 py-1 text-xs rounded border"
                  style={{ borderColor: "rgba(255,255,255,0.12)", color: "#fff" }}
                >
                  Select...
    ...
    <div className="h-screen w-screen">
      ...
      <div className="h-full flex" style={{ background: "#08121A", paddingTop: "32px" }}>
        <Sidebar
          ...
          openProxyTool={handleOpenProxyTool}
          openingProxyTool={openingProxyTool}
          proxyToolConfigured={!!proxyToolPath}
          openOpiSimulator={handleOpenOpiSimulator}
          openingOpiSimulator={openingOpiSimulator}
          opiSimulatorConfigured={!!opiSimulatorPath}
